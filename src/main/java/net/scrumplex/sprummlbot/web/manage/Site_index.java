	...
	public Site_index() {
		...
		try {
			...
			if (Vars.AVAILABLE_LOGINS.size() > 1) {
				...
				sb.append("            <p>There are ").append(Vars.AVAILABLE_LOGINS.size())
						.append(" Accounts available!</p>");
				sb.append("        </div>");
			...
			sb.append("        <p style=\"color:#fff\">There are currently ")
					.append(Vars.API.getServerInfo().get().getClientsOnline() - 1)
					.append(" user(s) and the Sprummlbot on your server!</p>");
			sb.append("    </div>");
			sb.append("");
			sb.append(
			...
			sb.append("        <h5 style=\"color:#fff\">Bans</h5>");
			int bancount = 0;
			List<Ban> bans = Vars.QUERY.getApi().getBans();
			if (bans != null)
				bancount = bans.size();
			sb.append("        <p style=\"color:#fff\">There are currently ").append(bancount)
					.append(" ban entries!</p>");
			if (bancount != 0)
			...
			sb.append("        console.log(encodeURIComponent(msg));");
			sb.append(
					"        location.href = '/manage/action/poke/!userid=' + id + ',msg=' + encodeURIComponent(msg);");
			sb.append("    }");
			sb.append("");
